...
void btree::destroy_leaf_keyval(int key , node *rot)
{
  ...
  if(( key==rot->key_value ))
  {
    ...
    else
    {
      ...
      rot->g=seed->g;
      rot->h=seed->h;
      if(!(rot->right->left||rot->right->right))
                rot->right=NULL;
...
closedtree.destroy_leaf_keyval(100249,closedtree.root);
closedtree.destroy_leaf_keyval(100249,closedtree.root);
closedtree.destroy_leaf_keyval(100248,closedtree.root);
closedtree.destroy_leaf_keyval(100240,closedtree.root);
closedtree.destroy_leaf_keyval(100241,closedtree.root);
node *tmp;
tmp=closedtree.root;
if(closedtree.search_bool(100249,closedtree.root))
   cout<<"it still exists";
else
    cout<<"it is not there";
while(tmp)
{
cout<<endl<<"the key is "<<tmp->key_value<<endl;
tmp=tmp->right;
